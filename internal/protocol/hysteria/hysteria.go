...
func (s *Server) Start(cfg protocol.NodeConfig) error {
	...
	if cfg.TLS.CertFile == "" || cfg.TLS.KeyFile == "" {
		return fmt.Errorf("hysteria: node %d requires tls cert_file/key_file", cfg.NodeID)
	}

	tlsConfig, err := tlsutil.NewStdServerConfig(cfg.TLS.CertFile, cfg.TLS.KeyFile, []string{"hysteria"})
	if err != nil {
		return fmt.Errorf("hysteria: node %d: load cert: %w", cfg.NodeID, err)
	}
...
func (s *Server) UpdateUsers(users []protocol.User) error {
	...
	s.mu.Unlock()
	if svc == nil {
		return fmt.Errorf("hysteria: not started")
	}
