// Package tuic implements protocol.ProtocolServer for TUIC, a QUIC-based
// protocol, backed by sagernet/sing-quic/tuic.
package tuic
...
func init() {
	protocol.Register("tuic", func() protocol.ProtocolServer { return New() })
}

const udpIdleTimeout = 300 * time.Second

// Server is a TUIC protocol.ProtocolServer. Zero value is ready to use.
type Server struct {
	mu         sync.Mutex
	packetConn net.PacketConn
	service    *stuic.Service[int64]
...
func (s *Server) Name() string { return "tuic" }

// Start binds a UDP listener and begins serving TUIC sessions. Requires
// cfg.TLS.CertFile/KeyFile.
func (s *Server) Start(cfg protocol.NodeConfig) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.packetConn != nil {
		return fmt.Errorf("tuic: node %d already started", cfg.NodeID)
	}
	if len(cfg.Users) == 0 {
		return fmt.Errorf("tuic: node %d has no users configured", cfg.NodeID)
	}
	if cfg.TLS.CertFile == "" || cfg.TLS.KeyFile == "" {
		return fmt.Errorf("tuic: node %d requires tls cert_file/key_file", cfg.NodeID)
	}

	tlsConfig, err := tlsutil.NewStdServerConfig(cfg.TLS.CertFile, cfg.TLS.KeyFile, []string{"h3"})
	if err != nil {
		return fmt.Errorf("tuic: node %d: load cert: %w", cfg.NodeID, err)
	}

	ctx, cancel := context.WithCancel(context.Background())
	service, err := stuic.NewService[int64](stuic.ServiceOptions{
		Context:    ctx,
		Logger:     logger.NOP(),
		TLSConfig:  tlsConfig,
	...
	if err != nil {
		cancel()
		return fmt.Errorf("tuic: node %d: %w", cfg.NodeID, err)
	}

	userIDs := make([]int64, len(cfg.Users))
	uuids := make([][16]byte, len(cfg.Users))
	...
	service.UpdateUsers(userIDs, uuids, passwords)

	addr := net.JoinHostPort(cfg.ListenIP, strconv.Itoa(cfg.Port))
	packetConn, err := net.ListenPacket("udp", addr)
	if err != nil {
		cancel()
		return fmt.Errorf("tuic: node %d: listen %s: %w", cfg.NodeID, addr, err)
	}
	if err := service.Start(packetConn); err != nil {
		packetConn.Close()
		cancel()
		return fmt.Errorf("tuic: node %d: start: %w", cfg.NodeID, err)
	}
...
// NewConnectionEx implements TUIC's ServiceHandler (N.TCPConnectionHandlerEx).
...
// NewPacketConnectionEx implements TUIC's ServiceHandler
